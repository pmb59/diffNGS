...
for (j in 1:length(x$p.values)) {
  ...
  # get the FCs
  if (results$avg.C2[j] >= results$avg.C1[j]) {
    results$log2fc[j] <- log2((results$avg.C2[j] + 0.001) / (results$avg.C1[j] + 0.001))
  } # Increase
  if (results$avg.C2[j] < results$avg.C1[j]) {
    results$log2fc[j] <- log2((results$avg.C1[j] + 0.001) / (results$avg.C2[j] + 0.001))
  } # Decrease
}
